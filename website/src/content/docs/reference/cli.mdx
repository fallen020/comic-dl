---
title: CLI Reference
description: Every command and flag in comic-dl, plus JSON output and exit codes.
order: 500
section: Reference
---

Look up commands, flags, defaults, and JSON output shapes. The default command
downloads content when given a URL; see [Downloading](../../usage/basic/) for
worked examples.
...
instructions. See [Self-Management](../usage/self-update.mdx) and
[Site Support](../usage/site-support.mdx) for the per-installation behaviour.
...
Series in parallel still pass through the per-host rate limiter, so raising `update --parallel` never exceeds the politeness budget.
...
| `--solver` | `off` | Cloudflare solver (opt-in): `off` (default; challenged sites fail), `auto`, `impersonation`, `webview` |
...
`--json` implies non-interactive mode and uses the same exit codes as the human-facing run.
...
- [Exit Codes](../exit-codes/) — failure categories, interrupts, and batch results
- [Supported Sites](../supported-sites/) — URL patterns and per-site notes
