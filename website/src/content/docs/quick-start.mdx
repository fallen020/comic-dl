---
title: Quick Start
description: Install comic-dl, download a gallery, and read the archive it produces.
order: 30
...
Download a binary from [GitHub Releases](https://github.com/fallen020/comic-dl/releases), or build from source:
...
The commands below assume `comic-dl` is on your PATH. From a source checkout, replace `comic-dl` with `uv run python -m comic_dl`. See [Installation](../installation) for platform-specific details.
...
Replace `<gallery-url>` with a gallery or series page from a [supported site](../reference/supported-sites).
...
On Windows this follows the real Shell Folders value, so a OneDrive-redirected home still resolves correctly. Run `comic-dl config show` to print the exact directory for your machine.
...
Blank lines, `#` comments, and inline `# comments` after URLs are supported. Duplicate URLs are downloaded once.
...
- [Downloading](../usage/basic) — select chapters or run a batch
- [Output & Archives](../usage/output) — find files and choose a format
- [Library Management](../usage/library) — inspect downloads and update tracked series